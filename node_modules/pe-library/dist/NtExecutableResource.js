...
var NtExecutableResource = /** @class */ (function () {
    ...
    NtExecutableResource.from = function (exe) {
        ...
        var entry = exe.getSectionByEntry(ImageDirectoryEntry_1.default.Resource);
        ...
        if (entry) {
            var reloc = exe.getSectionByEntry(ImageDirectoryEntry_1.default.BaseRelocation);
        ...
        r.sectionDataHeader = entry ? functions_1.cloneObject(entry.info) : null;
        if (entry) {
            r.parse(entry);
        }
        return r;
    ...
    NtExecutableResource.prototype.getResourceEntriesAsString = function (type, id) {
        return this.entries
            ...
            .map(function (entry) { return [entry.lang, functions_1.binaryToString(entry.bin)]; });
    };
    /**
    ...
    NtExecutableResource.prototype.replaceResourceEntryFromString = function (type, id, lang, value) {
        var entry = {
            ...
            lang: lang,
            codepage: 1200,
            bin: functions_1.stringToBinary(value),
        };
    ...
    NtExecutableResource.prototype.generateResourceData = function (virtualAddress, alignment, noGrow, allowShrink) {
        ...
        size = functions_1.roundUp(size, 8);
        var descOffset = size;
        ...
        var dataOffset = size;
        size = this.entries.reduce(function (p, e) {
            return functions_1.roundUp(p, 8) + e.bin.byteLength;
        }, dataOffset);
        var alignedSize = functions_1.roundUp(size, alignment);
        var originalAlignedSize = functions_1.roundUp(this.originalSize, alignment);
        if (noGrow) {
        ...
        this.entries.forEach(function (e) {
            var len = e.bin.byteLength;
            functions_1.copyBuffer(bin, o, e.bin, 0, len);
            o += functions_1.roundUp(len, 8);
        });
        var stringsData = [];
        o = stringsOffset;
    ...
    NtExecutableResource.prototype.outputResource = function (exeDest, noGrow, allowShrink) {
        ...
        if (this.sectionDataHeader) {
            sectionData = {
                data: null,
                info: functions_1.cloneObject(this.sectionDataHeader),
            };
        }
        else {
            sectionData = {
        ...
        // write as section
        exeDest.setSectionByEntry(ImageDirectoryEntry_1.default.Resource, sectionData);
        // rewrite section raw-data
        var generatedSection = exeDest.getSectionByEntry(ImageDirectoryEntry_1.default.Resource);
